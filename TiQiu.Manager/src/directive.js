angular.module('tiqiu')
            ...
            attrs.$observe('bookMenu', function(val) {
              ...
              if (scope.scheduleItem.ScheduledID) {
                var isExpire = scope.scheduleItem.ScheduledDate.slice(0, 10) + scope.scheduleItem.StartTime.slice(0, 5) < moment().format("YYYY-MM-DD").slice(0, 10) + moment().format("HH:mm"),
                  options = [];
                if (isExpire && scope.scheduleItem.OrderStatus < 2) {
  ...
  .directive("unread", function($window, $timeout, Order) {
    return {
      link: function(scope, element, attrs) {
        var snd = new Audio("notify.mp3"); // buffers automatically when created

        function refresh() {
          Order.getOrderRequestList(1, 1)
            .then(function(data) {
              var count = +data.Total || 0;
              if (count > 0) {
                snd.play();
                element.attr('data-content', data.Total).addClass('unread');
              } else {
