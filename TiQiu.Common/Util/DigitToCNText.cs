...
{
    public static class DigitToCNText
    {
        private static readonly char[] m_CNText = new char[] { '零', '一', '二', '三', '四', '五', '六', '七', '八', '九' };
        private static readonly char[] m_Digit = new char[] { '十', '百', '千', '万', '亿' };

        public static string Convert(string strDigit)
        {
            // 检查输入数字
            decimal dec;
            try
            ...
            {
                throw new ApplicationException("输入数字的格式不正确。");
            }
            catch (Exception e)
            {
                throw e;
            ...
            {
                throw new ApplicationException("输入数字太大或太小，超出范围。");
            }
        ...
        private static string ConvertIntegral(string strIntegral)
        {
            ...
            {
                strInt.Append(m_CNText[integral[i] - '0']);

                if (0 == digit % 4)     // '万' 或 '亿'
                {
                    if (4 == digit || 12 == digit)
                    {
                        strInt.Append(m_Digit[3]); // '万'
                    }
                    else if (8 == digit)
                    {
                        strInt.Append(m_Digit[4]); // '亿'
                    }
                }
                else         // '十'，'百'或'千'
                {
                    strInt.Append(m_Digit[digit % 4 - 1]);
                }
        ...
        private static string ConvertFractional(string strFractional)
        {
            ...
            {
                strFrac.Append(m_CNText[fractional[i] - '0']);
            }
